...
class dBug {
    ...
    function varIsArray($var) {
        array_push($this->arrHistory, $this->getVarHash($var));
        ...
        if (is_array($var)) {
            foreach ($var as $key => $value) {
                ...
                //check for recursion
                if (is_array($value)) {
                    if (in_array($this->getVarHash($value), $this->arrHistory, TRUE)) {
                        $value = "*RECURSION*";
                    }
                }
    ...
    protected function printObjectVars($arrObjVars, $prefix = '') {
        foreach ($arrObjVars as $key => $value) {
            $value = (!is_object($value) && !is_array($value) && trim($value) == "") ? "[empty string]" : $value;
            $this->makeTDHeader("object", $prefix . $key);
     ...
     * @param string $prefix
     */
    protected function printObjectProps($obj, $properties, $prefix = '') {
        /** @var ReflectionProperty $propertyInfo */
        foreach ($properties as $propertyInfo) {
            $propertyInfo->setAccessible(true);
            $propertyValue = $propertyInfo->getValue($obj);
            $propertyInfo->setAccessible(false);
            $propertyValue = (!is_object($propertyValue) && !is_array($propertyValue) && trim($propertyValue) == "") ? "[empty string]" : $propertyValue;
            $this->makeTDHeader("object", $prefix . $propertyInfo->getName());

            //check for recursion
            if (is_object($propertyValue) || is_array($propertyValue)) {
                if (in_array($this->getVarHash($propertyValue), $this->arrHistory)) {
                    $propertyValue = (is_object($propertyValue)) ? "*RECURSION* -> $" . get_class($propertyValue) : "*RECURSION*";

                }
            }
            if (in_array(gettype($propertyValue), $this->arrType)) {
                $this->checkType($propertyValue);
            } else {
                echo $propertyValue;
            }
            echo $this->closeTDRow();
    ...
    protected function getVarHash($objOrArray) {
        if (is_object($objOrArray)) {
            return sha1('object:' . spl_object_hash($objOrArray));
        } else if (is_resource($objOrArray)) {
            return get_class($objOrArray);
        } else {
            return sha1(json_encode($objOrArray));
        }
    }
