...
class DbObject {
     ...
     * @param bool $strictMode - used only when $data not empty and is array
     ...
     * @return $this
     */
    static public function create($dataOrPkValue = null, $strictMode = false, $isDbValues = false, $model = null) {
        $className = get_called_class();
        return new $className($dataOrPkValue, $strictMode, $isDbValues);
    }
     ...
     * @throws DbObjectException
     */
    public function __construct($dataOrPkValue = null, $strictMode = false, $isDbValues = false, $model = null) {
        if (!empty($model)) {
            if (!is_object($model)) {
        ...
        if (!empty($dataOrPkValue)) {
            if (is_array($dataOrPkValue)) {
                $this->_fromData($dataOrPkValue, !empty($strictMode), $isDbValues);
            } else {
     ...
     * @param bool|array $strictMode -
     *      true: filters data that does not belong to this object
     *      false: data that does not belong to this object will trigger exceptions
     *      array: list of fields to use
     * @return $this
     * @throws DbObjectException when $strictMode == false and unknown field detected in $data
     */
    public function fromData($data, $strictMode = false) {
        $this->_fromData($data, $strictMode, false);
        return $this;
    }
     ...
     * @throws DbObjectException when $strictMode == false and unknown field detected in $data
     */
    public function updateValues($values, $strictMode = false) {
        if ($this->exists()) {
            $values[$this->_getPkFieldName()] = $this->_getPkValue();
        }
        $this->_fromData($values, $strictMode, false);
        return $this;
    }
     ...
     * @throws DbObjectException when $strictMode == false and unknown field detected in $data
     */
    public function fromDbData($data, $strictMode = false) {
        $this->_fromData($data, $strictMode, true);
        return $this;
    }
     ...
     * @throws DbObjectException when $strictMode == false and unknown field detected in $data
     */
    protected function _fromData($data, $strictMode = false, $isDbValues = false) {
        ...
        if (is_array($data)) {
            ...
            // filter fields
            if (is_array($strictMode)) {
                $data = array_intersect_key($data, array_flip($strictMode));
                $strictMode = false;
            }
            ...
            foreach ($data as $fieldNameOrAlias => $value) {
                ...
                } else if (!$strictMode && $fieldNameOrAlias[0] !== '_') {
